# Redes neuronales, DeepLearning
## [Redes Neuronales](./1_RN.md)

Introducción. Juego.

## [IA Generativa](2_generativa.md)

Reflexiones sobre la creatividad.

## [GAN y Diffusion](./3_GAN.md)

Cómo funcionan las Generative Adversarial Networks. Conceptos de diffusion models.

## [Modelos grandes de lenguaje](./4_llm.md)

"Attention is all you need."

## [Deepfakes](./5_deepfakes.md)

Qué es un deepfake. Ejemplos

[🔙 Inicio de la unidad](../readme.md)
